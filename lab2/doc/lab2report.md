...
Для игнорирования директории `build` она была добавлена в `.gitignore`-файл.

## Вывод

Была написана программа на 2 языках - Python и C++. Программа на C++ была скомпилирована g++ компилятором. Обе программы могут измерять время выполнения одинакового вычисления. Сравнение быстродействия двух ЯП с помощью написанной программы в таблице:

|Размер цикла   |Время (Python)   |Время (C++)    |
|:--------------|:----------------|:--------------|
| 10            | 12 мкс          | 2 мкс         |
| 100           | 35 мкс          | 2 мкс         |
| 1000          | 260 мкс         | 4 мкс         |
| 10000         | 5 мс            | 26 мкс        |
| 100000        | 55 мс           | 250 мкс       |
| 1000000       | 540 мс          | 2.5 мс        |
| 10000000      | 5.2 с           | 25 мс         |

Из таблицы видно, что C++ работает на порядки быстрее, что и ожидалось.