...
$$M_{sim}=O(N_{roads} + N_{nodes} + N_{cars} + k_{time}) \qquad T_{sim}=O(k_{time}(\cdot N_{cars} + N_{nodes}))$$
...
$$M_{rs}=M_{sim} \qquad T_{rs} = T_{sim}\cdot w = O(k_{time}(\cdot N_{cars} + N_{nodes}) \cdot  w)$$

$w$ - количество итераций запуска случайного подбора параметров.
