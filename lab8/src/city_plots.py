...
CROSS_ROADS = {
    0: CrossRoad(
        coords=(0, 0), road_id_vs_ports=[0, 1, -1, -1], tr_light_duration_s=20.0
    ),
    1: CrossRoad(
        coords=(0.0, 300.0), road_id_vs_ports=[0, 2, -1, -1], tr_light_duration_s=15
    ),
    2: CrossRoad(
        coords=(400.0, 300.0), road_id_vs_ports=[1, 2, 3, 4], tr_light_duration_s=22
    ),
    3: CrossRoad(
        coords=(450.0, 500.0), road_id_vs_ports=[3, 5, -1, -1], tr_light_duration_s=18
    ),
    4: CrossRoad(
        coords=(600.0, 300.0), road_id_vs_ports=[5, 4, -1, -1], tr_light_duration_s=27
    ),
}
ROADS = {
    0: Road((0, 0), (1, 0)),
...
def plot_spb_map(cross_roads: List[CrossRoad], roads: List[Road]) -> None:
    ...
    plt.figure()
    plt.imshow(
        image, extent=(img_x_min, img_x_max, img_y_min, img_y_max), origin="upper"
    )
    ...
    for i, road in enumerate(roads):
        ...
        plt.text(
            (cr0.x + cr1.x) / 2 + offset_x,
            (cr0.y + cr1.y) / 2 + offset_y,
            str(i)
            + ", {"
            + str(road.node_from_id)
            + "|"
            + str(road.node_from_port_id)
            + ", "
            + str(road.node_to_id)
            + "|"
            + str(road.node_to_port_id)
            + "}",
            fontsize=8,
            color="orangered",
        )
...
def plot_spb_map_with_traffic(
    ...
    for i, road in enumerate(roads):
        ...
        plt.plot(
            np.array([cr0.x + n_vec[0], cr1.x + n_vec[0]], dtype=np.float32),
            np.array([cr0.y + n_vec[1], cr1.y + n_vec[1]], dtype=np.float32),
